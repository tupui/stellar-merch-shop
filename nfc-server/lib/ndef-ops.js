...
export class NDEFOperations {
  ...
  async readNDEFViaAPDU() {
    ...
    console.log(`readNDEFViaAPDU: SELECT NDEF Application response: ${response.toString('hex')}`);
    ...
    const selectFile = Buffer.from([0x00, 0xA4, 0x00, 0x0C, 0x02, ...NDEF_FILE_ID]);
    response = await reader.transmit(selectFile, 40);
    console.log(`readNDEFViaAPDU: SELECT NDEF File response: ${response.toString('hex')}`);
    if (response.length < 2 || response[response.length - 2] !== 0x90 || response[response.length - 1] !== 0x00) {
    ...
    while (ndefDataBuffer.length < nlen) {
      ...
      console.log(`readNDEFViaAPDU: Reading chunk at offset ${currentOffset}, length ${bytesToRead}...`);
  ...
  async writeNDEFViaAPDU(ndefMessage) {
    ...
    console.log('writeNDEFViaAPDU: Waiting for connection stability after previous operations...');
    ...
    try {
      ...
      console.log(`writeNDEFViaAPDU: SELECT NDEF Application response: ${response.toString('hex')}`);
    } catch (error) {
    ...
    const selectFile = Buffer.from([0x00, 0xA4, 0x00, 0x0C, 0x02, ...NDEF_FILE_ID]);
    response = await reader.transmit(selectFile, 40);
    console.log(`writeNDEFViaAPDU: SELECT NDEF File response: ${response.toString('hex')}`);
    if (response.length < 2 || response[response.length - 2] !== 0x90 || response[response.length - 1] !== 0x00) {
      console.log('writeNDEFViaAPDU: Standard SELECT failed, trying alternative...');
      const selectFileAlt = Buffer.from([0x00, 0xA4, 0x00, 0x00, 0x02, ...NDEF_FILE_ID]);
      response = await reader.transmit(selectFileAlt, 40);
      console.log(`writeNDEFViaAPDU: Alternative SELECT response: ${response.toString('hex')}`);
      if (response.length < 2 || response[response.length - 2] !== 0x90 || response[response.length - 1] !== 0x00) {
        throw new Error(`Failed to select NDEF file: status=${response.slice(-2).toString('hex')}`);
      }
    }
    ...
    const readNlen = Buffer.from([0x00, 0xB0, 0x00, 0x00, 0x02]);
    response = await reader.transmit(readNlen, 4);
    console.log(`writeNDEFViaAPDU: Read NLEN response: ${response.toString('hex')}`);
    if (response.length >= 2 && response[response.length - 2] === 0x90 && response[response.length - 1] === 0x00) {
      const currentNlen = response.readUInt16BE(0);
      console.log(`writeNDEFViaAPDU: Current NLEN: ${currentNlen} bytes`);
    } else {
      console.log(`writeNDEFViaAPDU: Could not read NLEN: status=${response.slice(-2).toString('hex')}`);
    }
    
    // Step 3: Write NDEF data at offset 2
    console.log(`writeNDEFViaAPDU: Step 3 - Writing ${ndefMessage.length} bytes of NDEF data at offset 2...`);
    ...
    console.log(`writeNDEFViaAPDU: Write data response: ${response.toString('hex')}`);
    if (response.length < 2 || response[response.length - 2] !== 0x90 || response[response.length - 1] !== 0x00) {
    ...
    // Step 4: Update NLEN
    console.log(`writeNDEFViaAPDU: Step 4 - Updating NLEN to ${ndefMessage.length}...`);
    ...
    console.log(`writeNDEFViaAPDU: Update NLEN response: ${response.toString('hex')}`);
    if (response.length < 2 || response[response.length - 2] !== 0x90 || response[response.length - 1] !== 0x00) {
      const statusHex = response.slice(-2).toString('hex');
      if (statusHex === '6985') {
        console.warn('writeNDEFViaAPDU: NLEN update failed with 6985 (chip may be locked), but data was written');
      } else {
  ...
  async writeNDEF(url, maxRetries = 3) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.log(`writeNDEF: Verification read returned: ${verifyUrl || 'null'}`);
        
        if (verifyUrl === null) {
          throw new Error(`Write verification failed: wrote "${urlToWrite}" but read back null. The write may not have persisted.`);
        } else if (verifyUrl !== urlToWrite) {
          throw new Error(`Write verification failed: wrote "${urlToWrite}" but read "${verifyUrl}"`);
        }
        
        console.log(`writeNDEF: Verification successful! Wrote and verified: ${urlToWrite}`);
        return urlToWrite;
      } catch (error) {
        lastError = error;
        const errorMessage = error.message || String(error);
        ...
        console.warn(`NDEF write attempt ${attempt}/${maxRetries} failed, retrying...`, errorMessage);
        await new Promise(resolve => setTimeout(resolve, 200 * attempt));
      }
    }
